...
from agentchat.utils.file_utils import get_object_name_from_minio_url
        ...
        for server in all_servers:
            ...
            try:
                if app_settings.storage.mode == "minio":
                    logo = server.get("logo_url")
                    if logo:
                        if isinstance(logo, str) and (logo.startswith("http://") or logo.startswith("https://")):
                            ...
                            if base_netloc and parsed.netloc == base_netloc:
                                object_name = get_object_name_from_minio_url(logo)
                                server["logo_url"] = storage_client.sign_url_for_get(object_name)
                        elif isinstance(logo, str):
                            server["logo_url"] = storage_client.sign_url_for_get(logo)
            except Exception:
                pass
        return all_servers
